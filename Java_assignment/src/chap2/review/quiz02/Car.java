...
public class Car {
	...
	public void setSpeed(int speed) {
		...
		if(mode == 'D' || mode == 'R') {
			if(mode == 'R' && speed>40) {
				System.out.println("R모드는 40이상 속도를 낼 수 없습니다.");
				return;
			}
			System.out.println("속도를 " + speed + "로 세팅합니다.");
			this.speed = speed;
			
		} else {
			System.out.println("D 또는 R 모드로 변경하세요.");
			return;
		}
	}
	...
	private void injectOil() {
		System.out.println("엔진 오일이 주입됩니다.");
	}
	...
	public void engineStop() {
		...
		if(this.mode != 'P') {
			System.out.println("변속기 모드를 먼저 P로 변경해주세요.");
			return;
		} else {
			this.start = false;
