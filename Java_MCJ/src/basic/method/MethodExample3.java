...
public class MethodExample3 {
	...
	/*
	 ...
	 4. return이라는 키워드를 사용하며 return 뒤에 전달할 값을 지정합니다. (변수, 상수)
	 ...
	 6. 모든 메서드가 반환값이 있는 것은 아닙니다. 메서드 실행 후에
	  반환할 값이 딱히 없을 경우에는 return을 생략해도 됩니다.
	...
	static int add(int n1, int n2) {
		return n1 + n2;
//		System.out.println("야~"); return으로 인하여 강제로 종료 되었기 때문에 실행 불가
	}
...
//		return new int[] {n1+n2, n1-n2, n1*n2, n1/n2};
	...
	//	static double[] calcArrayTotal(double[] num) {
	//		int total = 0;
	...
	public static void main(String[] args) {
		...
		int r1 = add(3,8); // int r1 = 11; 과 같음
		int r2 = add(10,15); // int r2 = 25; 과 같음
		int r3 = add(add(4,6),add(7,8)); // int r2 = add(10,15); 와 동일한 결과 발생
		System.out.println(r3);

		int[] totalResult = operateTotal(30, 6);
		int[] totalResult2 = operateTotal(30, add(2,4));
		...
		/*
		 - 반환값이 없는 void 타입의 메서드는 값이 돌아오지 않기 때문에
		  실행 결과를 변수에 저장 할 수 없고, 다른 메서드의 매개값으로도 사용이 불가능 합니다.
		 - 반드시 단독 호출하여 사용해야 합니다.
		 - void 메서드는 단순히 동작을 지시하는 역할을 수행합니다. 
		 */
				
//		int r4 = multi(10,7);
//		System.out.println(multi(10,7)); //void는 텅 비어있기 때문에 값을 가져올 수 없음
		multi(10, 7);
