...
public class BufferdWriterEx {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 - 버퍼: 데이터를 입력 혹은 출력하기 전에 임시로 저장해 주는 배열 형태의 공간.
		 
		 # 버퍼를 이용해야 하는 이유
		 - 버퍼를 사용하지 않으면 입출력이 너무 자주 일어난다.
		 ex)
		   "Java Programing"을 쓰는 경우
		...
		try {
			...
			try {
				bw.close(); // 가장 나중에 생성된 객체부터 먼저 클로즈 권장.
				fw.close();
				sc.close();
			} catch (IOException e) {
				e.printStackTrace();
